...
| 2026-09-07 | modulo 00 + slide 00 | `clarification` / `slides` | Introdotti client e server ai livelli hardware/software, protocolli HTTP e WebSocket, servizi, API/Web API, REST, risorse, URL, route e metodi HTTP; aggiunti quattro diagrammi vettoriali, inclusa la mappatura tra le regole di un protocollo e una conversazione, e il flusso completo di pubblicazione di un post. | Costruire il modello mentale client–server prima dei boundary e rendere visibile la catena frontend → protocollo → API/backend → database con un linguaggio introduttivo. | no |
