...
|---|---|---|---|---|---|
| 2026-09-08 | moduli 02–18 + standard editoriale | `clarification` | Uniformate tutte le lezioni al telaio della lezione 01 con orientamento iniziale, gerarchia dei titoli, formato HTML/Markdown e indice incrociato delle fonti ufficiali; ampliati CSS di base, form Bootstrap, controllo di flusso JavaScript, fondamenti SQL, lifecycle Vue e ciclo operativo del deploy; aggiunti rapporto di audit, diagramma riutilizzabile della profondità documentale e controllo automatico del formato. | Rendere ogni lezione autonoma, navigabile e verificabile rispetto alle fonti ufficiali senza misurare la qualità dalla sola lunghezza o copiare interi manuali. | no |
| 2026-09-08 | guida MDN + moduli 01–05, 08–11 e 13 | `clarification` | Creata una guida trasversale illustrata per riconoscere la struttura di MDN, leggere reference HTML/CSS/JavaScript/Web API/HTTP, interpretare sintassi, stato, Baseline, compatibilità e specifiche e applicare un processo di consultazione verificabile; sostituite le ripetizioni nelle lezioni con pannelli contestuali che indicano fonti, profondità e prodotto atteso. | Trasformare MDN da raccolta di link a strumento di lavoro comune al corso, mantenendo nelle singole lezioni la selezione didattica specifica. | no |
...
| 2026-09-07 | modulo 00 + slide 00 | `clarification` / `slides` | Introdotti client e server ai livelli hardware/software, protocolli HTTP e WebSocket, servizi, API/Web API, REST, risorse, URL, route e metodi HTTP; aggiunti dieci diagrammi vettoriali, inclusi la mappatura tra le regole di un protocollo e una conversazione, il rapporto tra server, Web API e web service, quattro schemi dedicati ai concetti di API, il flusso completo di una richiesta REST e quello di pubblicazione di un post; impaginata la dispensa con paragrafi HTML giustificati, immagini centrate, callout semantici e pannelli `details/summary` coerenti con il corso 2cornot2c. | Costruire il modello mentale client–server prima dei boundary e rendere visibile la catena frontend → protocollo → API/backend → database con un linguaggio introduttivo e una gerarchia grafica uniforme. | no |
