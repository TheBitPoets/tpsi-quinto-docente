...
def test_feisbuc_express_reference_preserves_rest_contract_and_error_model() -> None:
    ...
    with RunningServer(C_ROOT / "solution" / "src" / "server.js") as server:
        ...
        status, _, posts = request(f"{server.base}/api/posts")
        assert status == 200
        assert len(posts) == 1
...
def test_feisbuc_express_reference_keeps_store_replaceable_and_middleware_ordered() -> None:
    ...
    positions = [app.index(fragment) for fragment in fragments]
...
def test_express_debug_solution_fixes_pipeline_and_preserves_safe_get() -> None:
    ...
    with RunningServer(D_ROOT / "solution" / "server.js") as server:
        status, _, html = request(f"{server.base}/")
        assert status == 200
        assert "Debug Express" in html
        status, _, posts = request(f"{server.base}/api/posts")
        assert status == 200
        before = len(posts)
        with pytest.raises(urllib.error.HTTPError) as create_get:
            urllib.request.urlopen(f"{server.base}/api/posts/create?text=x", timeout=5)
        assert create_get.value.code == 404
        status, _, after_posts = request(f"{server.base}/api/posts")
        assert status == 200
        assert len(after_posts) == before
