...
def test_sources_project_exactly_to_course_design_catalog() -> None:
    ...
    assert normalized[0].files == (
        ...
        "17_TESTING_INTEGRATION_BOUNDARIES.md",
        "18_RUNTIME_DEPLOY_HEALTH_CAPSTONE.md",
    )
...
def test_external_specs_docs_and_licensed_material_are_references_not_sources() -> None:
    ...
    for provider in (
        ...
        "socketio", "react", "fastapi", "sqlalchemy", "pytest",
    ):
...
def test_content_items_are_ordered_and_linked_to_real_files() -> None:
    ...
    assert [item["order"] for item in items] == list(range(1, 20))
    for item in items:
    ...
    assert {
        ...
        "tpsi5-content-testing-integration-boundaries",
        "tpsi5-content-runtime-deploy-capstone",
    } <= ids
