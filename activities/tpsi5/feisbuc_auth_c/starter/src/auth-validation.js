...
export function validateRegistration(input) {
  ...
  if (Array.from(displayName).length < 1 || Array.from(displayName).length > 80) errors.push("display-name-invalid");
  const length = Array.from(password).length;
  if (length < 15) errors.push("password-too-short");
  if (length > 128) errors.push("password-too-long");
  return errors.length ? { ok: false, errors } : { ok: true, value: { email, displayName, password } };
}
