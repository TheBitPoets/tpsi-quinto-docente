...
export function loadConfig(env = process.env) {
  ...
  if (production && !cookieSecure) throw new Error("In production COOKIE_SECURE deve essere true");
  const sessionTtlMs = Number(env.SESSION_TTL_MS ?? 8 * 60 * 60 * 1000);
  if (!Number.isSafeInteger(sessionTtlMs) || sessionTtlMs < 60_000) throw new Error("SESSION_TTL_MS non valida");
  return {
    port,
    nodeEnv,
