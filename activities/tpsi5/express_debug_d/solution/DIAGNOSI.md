...
| 5 | `/explode` produce default 500 invece del nostro JSON | response `text/html` / stack dev | handler custom ha tre argomenti e non viene riconosciuto come error middleware | firma `(error, req, res, next)` e posizione finale | 500 `application/json` |
