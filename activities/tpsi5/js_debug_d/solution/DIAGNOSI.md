...
2. **Lo storage viene letto come array senza JSON.parse**: `localStorage.getItem()` restituisce una stringa o `null`; `saved.forEach` non è quindi valido. Fix: parse esplicito e controllo `Array.isArray`.
...
5. **L'identità appartiene ai controlli tramite `counter`**: `post_0`/`like_button_0` legano il modello all'ordine DOM e allo stato globale. Fix: id del post (`crypto.randomUUID`) + `data-post-id`/`data-action`.
...
8. **La persistenza non ha recovery**: JSON assente/corrotto può interrompere il caricamento. Fix: `try/catch`, valore `[]` di fallback e controllo del tipo.
