...
export function createPostsRouter({ postStore, postEvents }) {
  ...
  router.post("/", requireJson, (req, res) => {
    ...
    postEvents.publish({ type: "post:created", post: created });
  ...
  router.patch("/:id", requireJson, (req, res) => {
    ...
    postEvents.publish({ type: "post:updated", post: updated });
  ...
  router.delete("/:id", (req, res) => {
    ...
    if (result.status === "forbidden") {
      throw new HttpError(403, "forbidden", "Non puoi eliminare il post di un altro utente.");
    }
    postEvents.publish({ type: "post:deleted", postId: req.params.id });
    res.status(204).end();
  });
