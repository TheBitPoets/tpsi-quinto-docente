...
export function createApp({ authStore, postStore, config, staticDir }) {
  const app=express();app.disable("x-powered-by");if(config.trustProxy)app.set("trust proxy",1);
  app.use(requestContext);app.use(requestLogger);app.use(requireSameOriginForUnsafe);app.use(express.json({limit:"32kb"}));app.use(loadAuth({authStore,config}));app.use(express.static(staticDir));
  app.use("/api/auth",noStore,createAuthRouter({authStore,config}));
  app.use("/api/posts",noStore,createPostsRouter({postStore}));
  // TODO: installVueSpa(app,{vueRoot:path.join(staticDir,"vue")}) PRIMA di notFound.
  void path; void installVueSpa;
